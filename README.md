...
`192.168.200.1` is treated as a shared/default controller address. Calibration
consumers do not auto-select files for that IP: GUI launches open a filtered
file chooser, while headless or direct node starts must provide
`calibration_file:=<path>`. Bin teach also requires an explicit
`platform_calibration_file` for that IP. Explicit selections supplied by Robot
Cell Orchestrator bypass the additional chooser.
