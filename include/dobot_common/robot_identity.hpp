...
inline constexpr const char * kSharedRobotIpAddress = "192.168.200.1";
...
inline bool requiresManualCalibrationSelection(const std::string &robot_ip_address)
{
  return trimCopy(robot_ip_address) == kSharedRobotIpAddress;
}
...
struct LatestRobotFileSelection
{
  ...
  void consider(
    ...
    const std::string &robot_ip_address)
  {
    if (requiresManualCalibrationSelection(robot_ip_address))
    {
      return;
    }
