...
private:
  ...
  static std::string defaultRuntimeSettingsFile(const std::string &profiles_dir)
  {
    ...
    {
      return (std::filesystem::path(profiles_dir) / "item_detect_runtime_settings.yaml").string();
    }
    const char *home = std::getenv("HOME");
    if (home == nullptr)
    {
      return "item_detect_runtime_settings.yaml";
    }
    return (std::filesystem::path(home) / ".ros" / "item_detect_runtime_settings.yaml").string();
  }
  ...
  static std::string defaultSelectedProfileExportFile(const std::string &profiles_dir)
  {
    if (profiles_dir.empty())
    {
      return "item_detect_selected_profile.txt";
    }
    return (std::filesystem::path(profiles_dir) / "item_detect_selected_profile.txt").string();
  }
