...
DEFAULT_ACCEL_PERCENT = 4
MIN_ACCEL_PERCENT = 1
MAX_ACCEL_PERCENT = 20
MIN_SERVO_INTERVAL_MS = 200
MAX_SERVO_INTERVAL_MS = 400
DEFAULT_SERVO_INTERVAL_MS = 200
MAX_TF_FRAMES = 120
...
POINT_A_REACH_CHECK_PERIOD_SEC = 0.05
POINT_A_REACH_LOG_PERIOD_SEC = 0.5
...
def angular_error_degrees(actual_deg: float, target_deg: float) -> float:
    """Return absolute shortest angular error from actual to target in degrees."""
    return abs(shortest_angle_delta_degrees(actual_deg, target_deg))


def pose_error_summary(actual_pose: Pose6, target_pose: Pose6) -> tuple[float, float, tuple[float, float, float]]:
    """Return XYZ Euclidean error, max orientation error, and raw XYZ deltas."""
    ...
    max_angle_error = max(
        angular_error_degrees(actual_pose[3], target_pose[3]),
        angular_error_degrees(actual_pose[4], target_pose[4]),
        angular_error_degrees(actual_pose[5], target_pose[5]),
    )
    return xyz_error, max_angle_error, (dx, dy, dz)
...
def accel_percent_to_ramp_fraction(acceleration_percent: int) -> float:
    ...
    return 0.45 - (0.35 * normalized)  # 1% -> 45% ramp, 20% -> 10% ramp.
...
def choose_equalized_segment_count(total_time_sec: float, requested_dt_sec: float) -> int:
    """Choose a segment count that removes short leftover final segments.

    The GUI slider still selects the desired interval, but instead of generating
    0.0, dt, 2*dt, ..., total_time and leaving a short final remainder, we
    choose one integer segment count and spread all segments evenly across the
    total time. This makes the sampled acceleration and deceleration sides
    symmetric and keeps the actual interval inside the allowed 200-400 ms range
    whenever the total time makes that possible.
    """
    min_dt_sec = MIN_SERVO_INTERVAL_MS / 1000.0
    max_dt_sec = MAX_SERVO_INTERVAL_MS / 1000.0

    target_segments = max(1, int(round(total_time_sec / requested_dt_sec)))
    min_segments = max(1, int(math.ceil(total_time_sec / max_dt_sec)))
    max_segments = max(1, int(math.floor(total_time_sec / min_dt_sec)))

    if max_segments < min_segments:
        return max(1, target_segments)

    return max(min_segments, min(max_segments, target_segments))
...
def build_linear_servop_plan(
    ...
    if total_time_sec < MIN_SERVO_INTERVAL_MS / 1000.0:
        raise ValueError(
            f'ServoP total time must be at least {MIN_SERVO_INTERVAL_MS} ms'
        )

    ramp_fraction = accel_percent_to_ramp_fraction(accel_percent)
    segment_count = choose_equalized_segment_count(total_time_sec, nominal_dt)
    actual_dt = total_time_sec / segment_count
    time_points: list[float] = [index * actual_dt for index in range(segment_count)]
    ...
    return ServoPTrajectoryPlan(
        ...
        planner_hz=1.0 / actual_dt,
        requested_interval_ms=clamped_interval_ms,
        actual_interval_ms=actual_dt * 1000.0,
...
class ServoPDebugNode(Node):
    ...
    def wait_until_pose_reached(
        ...
        label: str,
        status_callback,
        timeout_sec: float = POINT_A_REACH_TIMEOUT_SEC,
        position_tolerance_mm: float = POINT_A_POSITION_TOLERANCE_MM,
        orientation_tolerance_deg: float = POINT_A_ORIENTATION_TOLERANCE_DEG,
    ) -> bool:
        """Block until TCP feedback is close enough to the requested target pose.

        This prevents the ServoP stream from starting while the robot is still
        moving toward Point A after MovJ. The comparison uses Euclidean XYZ
        distance in Dobot feedback units, normally millimeters, and shortest
        angular error for rx/ry/rz so wrap-around values do not look like a
        false 360 degree error.
        """
        started = time.monotonic()
        last_log = 0.0
        last_xyz_error: float | None = None
        last_angle_error: float | None = None
        last_age: float | None = None

        while rclpy.ok():
            now = time.monotonic()
            actual_pose, pose_stamp = self.latest_pose()

            if actual_pose is None:
                if now - last_log >= POINT_A_REACH_LOG_PERIOD_SEC:
                    status_callback(f'Waiting for TCP feedback before ServoP stream to confirm {label}...')
                    last_log = now
            else:
                feedback_age = now - pose_stamp
                last_age = feedback_age
                if feedback_age > POINT_A_FEEDBACK_MAX_AGE_SEC:
                    if now - last_log >= POINT_A_REACH_LOG_PERIOD_SEC:
                        status_callback(
                            f'Waiting for fresh TCP feedback before ServoP stream: latest feedback age={feedback_age:.2f}s'
                        )
                        last_log = now
                else:
                    xyz_error, angle_error, xyz_delta = pose_error_summary(actual_pose, target_pose)
                    last_xyz_error = xyz_error
                    last_angle_error = angle_error
                    if xyz_error <= position_tolerance_mm and angle_error <= orientation_tolerance_deg:
                        status_callback(
                            f'{label} reached. Starting ServoP stream. '
                            f'XYZ error={xyz_error:.2f} mm, max angle error={angle_error:.2f} deg'
                        )
                        return True

                    if now - last_log >= POINT_A_REACH_LOG_PERIOD_SEC:
                        status_callback(
                            f'Waiting for {label} before ServoP stream: '
                            f'XYZ error={xyz_error:.2f} mm '
                            f'(dx={xyz_delta[0]:.2f}, dy={xyz_delta[1]:.2f}, dz={xyz_delta[2]:.2f}), '
                            f'max angle error={angle_error:.2f} deg'
                        )
                        last_log = now
            ...
            if elapsed >= timeout_sec:
                details = []
                if last_xyz_error is not None and last_angle_error is not None:
                    details.append(f'last XYZ error={last_xyz_error:.2f} mm')
                    details.append(f'last max angle error={last_angle_error:.2f} deg')
                if last_age is not None:
                    details.append(f'last feedback age={last_age:.2f}s')
                detail_text = '; '.join(details) if details else 'no valid TCP feedback received'
                status_callback(
                    f'Timeout waiting for {label} to be reached after {timeout_sec:.1f}s; '
                    f'{detail_text}. ServoP stream aborted.'
                )
                return False

            time.sleep(POINT_A_REACH_CHECK_PERIOD_SEC)

        status_callback(f'ROS shutdown while waiting for {label}; ServoP stream aborted')
        return False
    ...
    def _plan_summary(self, plan: ServoPTrajectoryPlan) -> str:
        ...
        return (
            f'Linear ServoP plan: {len(plan.points)} points, '
            f'requested interval={plan.requested_interval_ms} ms, '
            f'equalized actual interval={plan.actual_interval_ms:.1f} ms, '
            f'segments={plan.segment_count}, total={plan.total_time_sec:.3f}s, '
    ...
    def publish_tf_only(
        ...
        if len(plan.points) >= 3:
            first_dt = plan.points[1].time_sec - plan.points[0].time_sec
            last_dt = plan.points[-1].time_sec - plan.points[-2].time_sec
            status_callback(
                f'Equalized segment timing: first dt={first_dt:.3f}s, last dt={last_dt:.3f}s '
                f'(no short leftover final segment)'
            )
    ...
    def run_test(
        ...
        if MOVEJ_SETTLE_SEC > 0.0:
            status_callback(f'Waiting {MOVEJ_SETTLE_SEC:.1f}s for MovJ settle before checking Point A...')
            time.sleep(MOVEJ_SETTLE_SEC)

        if not self.wait_until_pose_reached(point_a, 'Point A', status_callback):
            return False

        send_points = plan.points[1:]
        status_callback(
            f'Streaming {len(send_points)} ServoP waypoints at equalized {plan.actual_interval_ms:.1f} ms intervals '
...
class ServoPDebugGui:
    def __init__(self, node: ServoPDebugNode) -> None:
        ...
        tk.Label(control_row, text='Acceleration level (%)').grid(row=1, column=0, sticky='w', pady=(10, 0))
    ...
    def _interval_ms_changed(self, value: str) -> None:
        ...
        self.interval_ms_label_var.set(
            f'target {interval_ms} ms; planner equalizes all segments to avoid a short final step (~{frequency_hz:.1f} Hz target)'
        )
    ...
    def _read_plan_inputs(self) -> tuple[Pose6, Pose6, float, int, int]:
        ...
        if servo_p_time < MIN_SERVO_INTERVAL_MS / 1000.0:
            raise ValueError(f'ServoP total time must be at least {MIN_SERVO_INTERVAL_MS} ms')
