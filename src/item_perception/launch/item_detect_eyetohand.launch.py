...
def _to_int(value: str, name: str) -> int:
    try:
        ...
        raise RuntimeError(
            f"[item_detect_eyetohand.launch] {name} must be an integer, got: {value!r}"
        ) from exc
...
def _show_missing_calibration_dialog(message: str) -> None:
    try:
        ...
        print(f"[item_detect_eyetohand.launch] Could not open GUI dialog: {exc}")
        print(message)
...
def _find_latest_calibration(calibration_dir: str) -> str:
    try:
        ...
        print(f"[item_detect_eyetohand.launch] Failed to search calibrations in {calibration_dir}: {exc}")
        return ""
...
def _launch_setup(context, *args, **kwargs):
    ...
    if use_calibration:
        if calibration_file:
            ...
            if not _calibration_file_is_usable(selected_file):
                msg = (
                    "[item_detect_eyetohand.launch] calibration_file is set but missing/empty: "
                    f"{selected_file}"
                )
            ...
            if not selected_file:
                msg = (
                    "[item_detect_eyetohand.launch] No non-empty eye-to-hand calibration YAML found in "
                    f"{calibration_dir}. Provide one via calibration_file:=<path>."
                )
                _show_missing_calibration_dialog(msg)
                raise RuntimeError(msg)
        print(f"[item_detect_eyetohand.launch] Using calibration file: {selected_file}")
    ...
    return [
        Node(
            ...
            executable="item_detect_eyetohand",
            name="item_detect_eyetohand",
            output="screen",
