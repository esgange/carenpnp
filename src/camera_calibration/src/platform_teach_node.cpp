...
public:
    ...
    static_broadcaster_(std::make_shared<tf2_ros::StaticTransformBroadcaster>(this))
  {
    ...
    max_observed_age_sec_ = declare_parameter<double>("max_observed_age_sec", 0.75);
    ...
    RCLCPP_INFO(
      get_logger(),
      "platform_teach ready. Platform=%s base=%s observed_board=%s output_dir=%s",
  ...
  bool observedBoardReady(std::string &reason) const
  {
    TransformStampedMsg observed;
    return lookupObservedBoard(observed, reason);
  }

  std::vector<std::string> statusLines(const std::string &requested_name) const
  {
    ...
    lines.push_back(
      "Board markers: " + std::to_string(marker_count) + "/" +
      std::to_string(marker_ids_.size()) + " visible (" + formatMarkerIds(marker_ids_) + ")");

    TransformStampedMsg observed;
    std::string reason;
    if (!lookupObservedBoard(observed, reason))
    {
      lines.push_back("Teach status: waiting for board pose");
    ...
    lines.push_back("Teach status: ready to save");
    lines.push_back(
  ...
  std::filesystem::path saveCalibration(const std::string &requested_name)
  {
    ...
    out << "    roll: " << (roll * kRadToDeg) << "\n";
    out << "    pitch: " << (pitch * kRadToDeg) << "\n";
    out << "    yaw: " << (yaw * kRadToDeg) << "\n";
    out << "  translation:\n";
    out << "    x: " << t.x << "\n";
    out << "    y: " << t.y << "\n";
    out << "    z: " << t.z << "\n";
...
private:
  ...
  double max_observed_age_sec_{0.75};
  ...
  bool lookupObservedBoard(TransformStampedMsg &observed, std::string &reason) const
  {
    try
    {
      observed = tf_buffer_.lookupTransform(
        base_frame_,
        observed_board_frame_,
        tf2::TimePointZero);
    }
    catch (const tf2::TransformException &ex)
    {
      reason = "Missing TF " + base_frame_ + " -> " + observed_board_frame_ + ": " + ex.what();
      return false;
    }
    ...
    if (stamp.nanoseconds() != 0)
    {
      ...
      if (age_sec > std::max(0.05, max_observed_age_sec_))
      {
        reason = "Observed board TF is stale: " + formatDouble(age_sec, 2) + "s.";
        return false;
      }
    }
    return true;
  ...
  bool loadCalibrationFromFile(const std::filesystem::path &path, PlatformCalibration &calibration, std::string &reason) const
  {
    ...
    {
      reason = "Missing calibration_transform rotation/translation";
      return false;
    }
